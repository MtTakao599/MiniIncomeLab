...
- **公開**: GitHub Pages または Cloudflare Pages（`public/` をそのまま公開）
...
v0.1 では「ミニPC比較サイト」の一覧ページ（`public/index.html`）のみを対象としています。将来的には中古PC、Linux、タイ移住準備、管理人便利グッズなどへ横展開できるよう、CSV + テンプレート構成を維持します。
...
- 生成物（`public/`）のみ GitHub Pages / Cloudflare Pages で公開する
...
- 商品ページの個別 HTML 生成（`public/products/` は将来用）
...
- [ ] `python scripts\build_site.py --clean --open` で `public\index.html` が生成され、ブラウザが開く
...
| オプション | 説明 |
|-----------|------|
| `--clean` | 再生成前に `public/index.html` など既知の生成物を削除 |
| `--open` | 生成後に既定ブラウザで `public/index.html` を開く |
...
生成物は `public/index.html` です。パスは `pathlib.Path` を使っているため、Windows / Linux どちらでも同じスクリプトが動きます。
...
`public/` 配下がそのまま公開ディレクトリになります。ビルド後の `public/index.html` がサイトのトップページです。

### GitHub Pages

1. リポジトリ Settings → Pages
2. **Build and deployment** → Source: **Deploy from a branch**
3. Branch: `main`（または利用ブランチ）、Folder: **`/public`**
4. ミニPC でビルド → `public/` を commit / push

`public/.nojekyll` により Jekyll 処理を無効化し、静的 HTML をそのまま配信します。
...
4. リポジトリに生成済み `public/` を含める運用でも、Cloudflare 上でビルドする運用でも可
